...
class Trie:
    ...
    def __init__(self):
        self.start = Node()
        # to avoid storing many (identical) strings as values, we use a mapping
        self.value_mapping = {}
...
def tokenise_fast(text, spacy_model='es_core_news_sm'):
    ...
    for token in tokens:
        ...
        # Special case: handle tokens like 3G, where Spacy tokenisation is unpredictable
            ...
            if not hasattr(tokenise_fast, "nlp"):
                tokenise_fast.nlp = spacy.load(
                    spacy_model, disable=["tagger", "parser", "ner"])
...
def generator_iob(doc):
    for t in doc:
        text = None
        if t.text.isspace():
            text = "\n"
        elif t.ent_type_:
            text = f"{t.text} {t.ent_iob_}-{t.ent_type_}\n"
        else:
            text = f"{t.text} O\n"
...
def export_iob(doc, filename: str, encoding: str = 'utf-8'):
    with open(filename, 'w', encoding=encoding) as f:
        f.writelines(generator_iob(doc))
