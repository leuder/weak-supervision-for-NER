...
POSITIONED_LABELS = ["O"] + ["%s-%s" %
                             (bilu, label) for label in LABELS for bilu in "BILU"]
...
NUMBER_NERS = ["CARDINAL", "DATE", "MONEY",
...
class UnifiedAnnotator(annotations.BaseAnnotator):
    ...
    def __init__(self, sources_to_keep=None, source_name="HMM"):
        ...
        if sources_to_keep is None:
            ...
            self.source_indices_to_keep = {
                SOURCE_NAMES.index(s) for s in sources_to_keep}
    ...
    def extract_sequence(self, doc):
        ...
        sequence = np.zeros((len(doc), len(SOURCE_NAMES), len(
            POSITIONED_LABELS)), dtype=np.float32)
            ...
            for (start, end), vals in doc.user_data["annotations"][source].items():
                for label, conf in vals:
                    ...
                    if end-start == 1:
                        ...
                        sequence[start+1:end-1, i,
                                 LABEL_INDICES["I-%s" % label]] = conf
                ...
                for label, conf in vals:
                    if label in {"ENT", "MISC"}:
                        ...
                        for other_source in other_sources:
                            overlaps = annotations.get_overlaps(
                                start, end, annotated, [other_source])
                            for (start2, end2, vals2) in overlaps:
                                for label2, conf2 in vals2:
                                    if label2 not in {"ENT", "MISC"}:
                                        ...
                                        conf2 = conf2 if (
                                            start2 == start and end2 == end) else 0.3*conf2
                                        conf2 = conf2 * \
                                            SOURCE_PRIORS[other_source][label2][0]
                                        label_counts[label2] = label_counts.get(
                                            label2, 0) + conf*conf2
...
class HMMAnnotator(hmmlearn.hmm._BaseHMM, UnifiedAnnotator):
    ...
    def __init__(self, sources_to_keep=None, source_name="HMM", informative_priors=True):
        hmmlearn.hmm._BaseHMM.__init__(
            self, len(POSITIONED_LABELS), verbose=True, n_iter=10)
        UnifiedAnnotator.__init__(
            self, sources_to_keep=sources_to_keep, source_name=source_name)
    ...
    def label(self, doc):
        ...
        predicted_proba = predicted_proba / \
            predicted_proba.sum(axis=1)[:, np.newaxis]

        confidences = np.array(
            [probs[x] for (probs, x) in zip(predicted_proba, predicted)])
    ...
    def _initialise_startprob(self, X_stream):
        ...
        if self.informative_priors:
            source_with_best_coverage = sorted(
                SOURCE_NAMES, key=lambda x: len(SOURCE_PRIORS[x]))[-1]
            print("Using source", source_with_best_coverage,
                  "to estimate start probability priors")
    ...
    def _initialise_transmat(self, X_stream):
        ...
        trans_counts = np.zeros(
            (len(POSITIONED_LABELS), len(POSITIONED_LABELS)))
        ...
        if self.informative_priors:
            ...
            for X in X_stream:
                for k in range(0, len(X)-1):
                    trans_counts[X[k, source_index].argmax(
                    ), X[k+1, source_index].argmax()] += 1
    ...
    def _initialise_emissions(self, X_stream, strength=1000):
        ...
        obs_counts = np.zeros(
            (len(SOURCE_NAMES), len(POSITIONED_LABELS)), dtype=np.float64)
        if self.informative_priors:
            for X in X_stream:
        ...
        matrix = np.zeros((len(SOURCE_NAMES), len(
            POSITIONED_LABELS), len(POSITIONED_LABELS)))
        ...
        for source_index, source in enumerate(SOURCE_NAMES):
            ...
            for pos_index, pos_label in enumerate(POSITIONED_LABELS):
                ...
                for pos_index2, pos_label2 in enumerate(POSITIONED_LABELS):
                    ...
                    error_prob = (1-recall) * (1-precision) * \
                ...
                error_indices = [i for i in range(
                    len(POSITIONED_LABELS)) if i != pos_index]
                error_sum = matrix[source_index,
                                   pos_index, error_indices].sum()
                matrix[source_index, pos_index,
                       error_indices] /= (error_sum/(1-recall))
    ...
    def generate_sample_from_state(self, state, random_state=None):
        result = np.zeros(
            (len(SOURCE_NAMES), len(POSITIONED_LABELS)), dtype=bool)
    ...
    def _compute_log_likelihood(self, X):
        ...
        for source_index in range(len(SOURCE_NAMES)):
            if source_index not in self.source_indices_to_keep:
                continue
            probs = np.dot(X[:, source_index, :],
                           self.emission_probs[source_index, :, :].T)
    ...
    def _do_mstep(self, stats):
        ...
        if 'e' in self.params:
            ...
            emission_probs = emission_counts / \
                (emission_counts + 1E-100).sum(axis=2)[:, :, np.newaxis]
            self.emission_probs = np.where(
                self.emission_probs > 0, emission_probs, 0)
...
def _check_outputs(predictions):
    ...
    for i in range(len(predictions)):
        ...
        if prev_bilu_label[0] in {"L", "U", "O"} and bilu_label[0] in {"I", "L"}:
            print("inconsistent start of NER at pos %i:" %
                  i, bilu_label, "after", prev_bilu_label)
        elif prev_bilu_label[0] in {"B", "I"}:
            if bilu_label[0] not in {"I", "L"} or bilu_label[2:] != prev_bilu_label[2:]:
                print("inconsistent continuation of NER at pos %i:" %
                      i, bilu_label, "after", prev_bilu_label)
            ...
            if source in doc.user_data["annotations"]:
                for (start, end), vals in doc.user_data["annotations"][source].items():
                    if (start, end) in span_indices and len(vals) > 0:
                        ...
                        obs[span_index, source_index] = 1 + \
                            LABELS.index(vals[-1][0])
