...
LEGAL_SUFFIXES = {
    ...
    # Akcionarsko Društvo (Serbia), Aktsionerno Drujestvo (Bulgaria) ~600
    'ad',
...
class Parser:
    ...
    def __init__(self, model, min_length_for_truecasing=30, correct_entities=True):
        ...
        self.nlp.add_pipe(_correct_tokenisation,
                          after="sentencizer", name="tokenize")
...
def _correct_tokenisation(parsed):
    ...
    while tokens_stack:
        ...
        match = re.match("(%s)([\d\.\,]+.*)" %
                         "|".join(CURRENCY_CODES), token.text)
        if match:
        ...
        match = re.match(
            "([\d\.\,]+)(bl?n?\.?|ml?n?\.?|k\.+)$", token.text, re.IGNORECASE)
        if match:
            tokens += [match.group(1), match.group(2)]
            spaces += [False, bool(token.whitespace_)]
            has_changed = True
...
def truecase(spacy_doc, min_prob=0.25, skip_doc_longer_than=25):
    ...
    spaces = [spacy_doc.text[tok.idx+len(tok)].isspace()
              for tok in spacy_doc[:-1]] + [False]
...
def _correct_entities(parsed, recursive=True):
    ...
    existing_ents = [
        ent for ent in parsed.ents if ent.text not in NOT_NAMED_ENTITIES]
