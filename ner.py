...
CHARACTERS = [chr(i) for i in range(32, 127)] + [chr(i)
...
class NERModel(annotations.BaseAnnotator):
    ...
    def __init__(self, name="neural-ner-model", model_file=None, **kwargs):
        self.params = {
            ...
            "vocab_size": 20000,
            # Whether to tune the word embeddings or keep them fixed
            "trainable_word_embeddings": True,
            # Dimension of the dense layer transforming the word embeddings (0 to skip)
            "word_emb_transform_dim": 0,
            # Dimension for the character embeddings (0 to skip the layer)
            "char_embedding_dim": 48,
            # Whether to normalise the characters in the tokens
            "normalise_chars": False,
            # Maximum number of characters per token (for the character embeddings)
            "max_token_length": 32,
            # Dimension for the character-level biLSTM (0 to skip the layer)
            "char_lstm_dim": 48,

            "use_roberta_embeddings": False,     # Whether to also include roberta embeddings

            # Number of token-level convolutional layers (0 for no layers)
            "nb_convo_layers": 0,
            ...
            # Dimension for the token-level biLSTM (0 to skip the layer)
            "token_lstm_dim": 128,
            # Size of the dense layer (after convolutions and LSTMs)
            "dense_dim": 0,
            # Whether to use a CRF as final layer (0 to skip the layer)
            "use_crf": False,

            # Dropout ratio (on all embeddings)
            "dropout": 0.3,
      ...
      #      tf.config.experimental.set_memory_growth(gpu, True)
        ...
        if model_file is not None:
            ...
            self.label_indices_inverted = {
                i: l for l, i in self.label_indices.items()}
    ...
    def train(self, train_docs, val_docs):
        ...
        self.model.fit_generator(batch_generator, validation_data=val_batch,
                                 ...
                                 epochs=self.params["nb_epochs"], callbacks=[
                                     saver],
    ...
    #    self._check_outputs(predictions)
        ...
        if replace is True:
            ...
            annotated = {(start, end): ((label, conf),)
    ...
    def _convert_inputs(self, spacy_doc):
        ...
        token_indices = np.array(
            [[self.indices.get(token.norm_, 1) for token in spacy_doc]], dtype=np.int32)
        inputs = [token_indices]

        # Then the character level indices
        if self.params["char_embedding_dim"]:
            char_inputs = np.zeros(
                (1, len(spacy_doc), self.params["max_token_length"]), dtype=np.int16)
            for token in spacy_doc:
                token2 = token.norm_ if self.params["normalise_chars"] else token.text
                token_length = min(
                    self.params["max_token_length"], len(token2))
                char_inputs[0, token.i, :token_length] = [self.char_indices.get(
                    c, 1) for i, c in enumerate(token2) if i < token_length]
    ...
    def _convert_outputs(self, spacy_doc):
        labels = np.zeros((1, len(spacy_doc), len(
            self.label_indices)), dtype=np.float32)
        for ent in spacy_doc.ents:
            ...
            if ent.start == ent.end-1:
                labels[0, ent.start, self.label_indices["U-%s" %
                                                        ent.label_]] = conf
            else:
                labels[0, ent.start, self.label_indices["B-%s" %
                                                        ent.label_]] = conf
                for i in range(ent.start+1, ent.end-1):
                    labels[0, i, self.label_indices["I-%s" % ent.label_]] = conf
                labels[0, ent.end-1, self.label_indices["L-%s" %
                                                        ent.label_]] = conf
...
def _stack_batch(data_batch):
    ...
    for i in range(nb_inputs):
        ...
        for j, (inputs_in_batch, _, _) in enumerate(data_batch):
            all_inputs_part[j, :len(inputs_in_batch[i][0])
                            ] = inputs_in_batch[i][0]
        all_inputs.append(all_inputs_part)

    all_targets = np.zeros(
        (len(data_batch), max_sequence_length, data_batch[0][1].shape[-1]), dtype=bool)

    # The default weight is near-zero to ignore padded values during training and evaluation
    all_weights = 0.0001 * \
        np.ones((len(data_batch), max_sequence_length), dtype=np.float32)
...
def generate_from_docbin(docbin_file, target_source=None, cutoff=None, nb_to_skip=0,
    ...
    while True:
        reader = annotations.docbin_reader(
            docbin_file, vocab=vocab, cutoff=cutoff, nb_to_skip=nb_to_skip)
            ...
            for start, end, label in spans:
                ...
                if labels_to_retain is None or label in labels_to_retain:
                    ent = spacy.tokens.Span(
                        spacy_doc, start, end, spacy_doc.vocab.strings[label])
