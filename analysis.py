...
def evaluate(docs, target_sources, labels_to_map=None, labels_to_keep=None):
    ...
    df["proportion"] = df.proportion.apply(
        lambda x: "%.1f %%" % (x*100) if not np.isnan(x) else "")
    df["token_cee"] = df.token_cee.apply(
...
def get_results(docs, target_source, labels_to_map=None, labels_to_keep=None, conf_threshold=0.5):
    ...
    for doc in docs:
        ...
        all_labels = {ent.label_ for ent in doc.ents} | {
            label for _, _, label in spans}
        for label in all_labels:
            true_spans = {(ent.start, ent.end)
                          for ent in doc.ents if ent.label_ == label}
            pred_spans = {(start, end)
                          for (start, end, label2) in spans if label2 == label}
            ...
            ent_tp[label] = ent_tp.get(label, 0) + \
                len(true_spans.intersection(pred_spans))
            ...
            true_tok_labels = {i for start,
                               end in true_spans for i in range(start, end)}
            pred_tok_labels = {i for start,
                               end in pred_spans for i in range(start, end)}
            tok_tp[label] = tok_tp.get(
                label, 0) + len(true_tok_labels.intersection(pred_tok_labels))
            tok_fp[label] = tok_fp.get(label, 0) + \
                len(pred_tok_labels - true_tok_labels)
            tok_fn[label] = tok_fn.get(label, 0) + \
                len(true_tok_labels - pred_tok_labels)
            tok_support[label] = tok_support.get(
                label, 0) + len(true_tok_labels)
    ...
    for label in ent_support:
        ...
        ent_f1_numerator = (
            results[label]["entity_precision"] * results[label]["entity_recall"])
        ent_f1_denominator = (
            results[label]["entity_precision"] + results[label]["entity_recall"]) + 1E-10
        results[label]["entity_f1"] = 2 * \
            round(ent_f1_numerator / ent_f1_denominator, 3)

        tok_f1_numerator = (
            results[label]["token_precision"] * results[label]["token_recall"])
        tok_f1_denominator = (
            results[label]["token_precision"] + results[label]["token_recall"]) + 1E-10
        results[label]["token_f1"] = 2 * \
            round(tok_f1_numerator / tok_f1_denominator, 3)
    ...
    label_weights = {l: ent_support[l] /
                     sum(ent_support.values()) for l in ent_support}
    ...
    ent_pred = sum([ent_tp[l] for l in ent_support]) + \
        sum([ent_fp[l] for l in ent_support]) + 1E-10
    ent_true = sum([ent_tp[l] for l in ent_support]) + \
        sum([ent_fn[l] for l in ent_support]) + 1E-10
    tok_pred = sum([tok_tp[l] for l in ent_support]) + \
        sum([tok_fp[l] for l in ent_support]) + 1E-10
    tok_true = sum([tok_tp[l] for l in ent_support]) + \
        sum([tok_fn[l] for l in ent_support]) + 1E-10
    ...
    for metric in ["macro", "weighted", "micro"]:
        ent_f1_numerator = (
            results[metric]["entity_precision"] * results[metric]["entity_recall"])
        ent_f1_denominator = (
            results[metric]["entity_precision"] + results[metric]["entity_recall"]) + 1E-10
        results[metric]["entity_f1"] = 2 * \
            round(ent_f1_numerator / ent_f1_denominator, 3)

        tok_f1_numerator = (
            results[metric]["token_precision"] * results[metric]["token_recall"])
        tok_f1_denominator = (
            results[metric]["token_precision"] + results[metric]["token_recall"]) + 1E-10
        results[metric]["token_f1"] = 2 * \
            round(tok_f1_numerator / tok_f1_denominator, 3)
...
def compute_logloss(doc, target_source, labels_to_map=None):
    ...
    all_labels = {ent.label_ for ent in doc.ents}
    pos_labels = ["O"] + ["%s-%s" % (bilu, label)
                          for label in sorted(all_labels) for bilu in "BILU"]
    pos_label_indices = {pos_label: i for i,
                         pos_label in enumerate(pos_labels)}
            ...
            if sent.text.strip() == v["original_text"].strip():
                for span in v["annotated_text"].split():
                    if "/" in span:
                        ...
                        ent_span = doc.char_span(
                            sent.start_char+start, sent.start_char+end)
                        if ent_span is None:
    ...
    for doc in crowd_docs:
        ...
        for (start, end) in sorted(doc.user_data["annotations"]["crowd"]):
            for val, conf in doc.user_data["annotations"]["crowd"][(start, end)]:
                ...
                if other_end > start:
                    print("overlap between", start,
                          end, other_start, other_end)
                    spans = spans[:-1]
                    start = other_start
                spans.append(spacy.tokens.Span(
                    doc, start, end, nlp.vocab.strings[val if val != "DATETIME" else "DATE"]))
